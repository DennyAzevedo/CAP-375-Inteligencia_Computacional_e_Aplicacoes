...
class LoadDataset:
    ...
    def loading(self):
        ...
        fulldataInput = np.loadtxt(url + "input.txt")

        fulldataOutput = np.loadtxt(url + "output.txt")

        dataset = np.column_stack([fulldataInput, fulldataOutput])

        dataset_norm = self.normalize(dataset)
